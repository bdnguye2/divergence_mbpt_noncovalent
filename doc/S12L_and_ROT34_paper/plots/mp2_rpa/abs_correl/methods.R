...
data1<-read.csv("elec_prod_correl.csv")
...
#png("mp2_rpa_correl.png", width = 9, height = 6.25, units = 'in', res = 300)
postscript("mp2_rpa_correl.eps", width =9, height = 6.25, paper="special", horizontal=FALSE)

par(mfrow=c(1,1),mar=c(5,5,2,2))
#Beginning of the Scatterplot 
plot(dataX,dataMP2,pch=1,col="black",lwd=2,
     xlab='',ylab='',
     xaxt="n",ylim=c(0,160),yaxt="n",cex.lab=1.4,cex.axis=1.4)
...
axis(2,yaxp=c(0,160,8),cex.axis=1.4,las=1)
title(ylab=expression(paste(Delta,italic(E^C),' (kcal/mol)')),cex.lab=1.4,line=3.2)
...
#Legend
legend("topleft",c("MP2","RPA(PBE)"),
       cex=1.2,col=c("black","blue"),
       pch=c(1,6),lty=c(1,3),bty='n',pt.cex=1.5,
