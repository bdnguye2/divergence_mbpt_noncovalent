...
%DIF DEL achemso.tex           Tue Feb  4 22:31:15 2020
%DIF ADD diff2/modified2.tex   Tue Feb  4 22:30:54 2020
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%DIF 77c77
%DIF < \author{Matthew Tang}
%DIF -------
\author{Matthew P. Tang} %DIF > 
%DIF -------
...
\begin{abstract}
  ...
  errors with the system size at a rate of approximately \DIFdelbegin \DIFdel{1\textperthousand\,
  }\DIFdelend \DIFaddbegin \DIFadd{0.1$\%$
  }\DIFaddend per valence electron, whereas the RPA and
...
\cite{doi:10.1080/00268970903397249}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{doi:10.1080/00268970903397249,doi:10.1007/s00214-011-1009-6}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend Comparing initially to solution phase thermodynamic
...
triggering the development of \DIFdelbegin \DIFdel{empirical }\DIFdelend \DIFaddbegin \DIFadd{dispersion }\DIFaddend corrections including MP2C,
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Pitonak10JChemTheoryComput6p168,doi:10.1063/1.4809981} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{doi:10.1063/1.2905808,Pitonak10JChemTheoryComput6p168,doi:10.1063/1.4809981}
}\hspace{0pt}%DIFAUXCMD
MP2D,\mbox{%DIFAUXCMD
\cite{doi:10.1021/acs.jctc.8b00548,C9SC05689K} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend as well as
empirical three-\cite{Grimme12ChemEurJ18p9955,Caldeweyher17JChemPhys147p034112},
and many-body\cite{Tkatchenko12PhysRevLett108p236402} \DIFdelbegin \DIFdel{empirical }\DIFdelend dispersion
...
the S66 benchmark set\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct200523a,doi:10.1021/ct200523a}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct2002946,doi:10.1021/ct200523a}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend and against the \DIFdelbegin \DIFdel{recently published }\DIFdelend domain-based local pair-natural orbital (DLPNO)
based CCSD(T) calculations for the L7 and S30L benchmark
sets.\cite{doi:10.1063/1.5090222,doi:10.1063/1.5012601}
\DIFdelbegin \DIFdel{The reported reference results were all extrapolated to }\DIFdelend \DIFaddbegin \DIFadd{DLPNO-CCSD(T) results can vary significantly for weakly bound
complexes depending on the truncation of the PNO basis and domain
size.\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct501129s,doi:10.1021/acs.jpclett.9b01156}
}\hspace{0pt}%DIFAUXCMD
...
basis-set extrapolation scheme as seen by Refs.~\mbox{%DIFAUXCMD
\citenum{doi:10.1063/1.5012601}
}\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citenum{doi:10.1021/acs.jpclett.9b01156}}\hspace{0pt}%DIFAUXCMD
. For the present study,
the DLPNO-CCSD(T) reference values were taken from Brandenburg
}\textit{\DIFadd{et al}}\DIFadd{\mbox{%DIFAUXCMD
\cite{doi:10.1063/1.5012601} }\hspace{0pt}%DIFAUXCMD
employing the ``TightPNO''
truncation thresholds\mbox{%DIFAUXCMD
\cite{doi:10.1021/ct501129s} }\hspace{0pt}%DIFAUXCMD
and the basis-set extrapolation scheme from
Ref.~\mbox{%DIFAUXCMD
\citenum{doi:10.1021/acs.jctc.5b00515}
}\hspace{0pt}%DIFAUXCMD
for both the L7 and S30L benchmarks. Throughout }\DIFaddend the \DIFdelbegin \DIFdel{CBS
limit. \mbox{%DIFAUXCMD
\cite{doi:10.1021/ct200523a, doi:10.1063/1.5090222}
}\hspace{0pt}%DIFAUXCMD
Throughout this }\DIFdelend paper,
...
were chosen for the KS \DIFdelbegin \DIFdel{and HF }\DIFdelend reference calculations because they were
found to yield 
...
\DIFaddbegin \DIFadd{Small core relativistic
effective core potentials\mbox{%DIFAUXCMD
\cite{doi:10.1063/1.1622924,doi:10.1021/jp065887l}
}\hspace{0pt}%DIFAUXCMD
were used for the halogen atoms in the S30L\mbox{%DIFAUXCMD
\cite{Sure15JChemTheoryComput}
}\hspace{0pt}%DIFAUXCMD
benchmark set.
}\DIFaddend The frozen core approximation was employed for the RPA correlation energy
...
correlation-consistent polarized triple- (cc-pVTZ) and quadruple-$\zeta$ \DIFaddbegin \DIFadd{(cc-pVQZ) }\DIFaddend valence basis
...
\DIFaddbegin \DIFadd{Similarly, correlation energies from MP2 and its variants were obtained
using the frozen core approximation, $50\%$ CP correction, and the 3-4 extrapolation 
using cc-pVTZ and cc-pVQZ
basis sets.\mbox{%DIFAUXCMD
\cite{Dunning89JChemPhys90p1007,doi:10.1063/1.464303}
}\hspace{0pt}%DIFAUXCMD
HF energies were computed using def2-QZVP
basis set.\mbox{%DIFAUXCMD
\cite{Weigend05PhysChemChemPhys7p3297,Weigend03JChemPhys119p12753}
}\hspace{0pt}%DIFAUXCMD
}
...
\begin{table}[hbtp]
  ...
  \end{tabular}}
  \begin{flushleft}
  \textsuperscript{\emph{a}} MP3 values from Refs.\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL \citenum{doi:10.1021/ct2002946}
...
\begin{figure}[hbtp]
  ...
  \caption{Errors in ROT34 computed rotational constants compared to
    ...
    \DIFaddbeginFL \DIFaddFL{PBE-D3, }\DIFaddendFL and M06L results are from Ref. \citenum{BrandenburgPhysRevB}.}
...
B. The supersystem eigenstates $\ket{\Psi_n^\alpha}$, which \DIFdelbegin \DIFdel{must }\DIFdelend \DIFaddbegin \DIFadd{are
constrained to }\DIFaddend be
antisymmetric under any permutations of electrons in the supersystem \DIFaddbegin \DIFadd{as
in conventional SAPT}\DIFaddend ,
...
closed-shell fragments. \DIFaddbegin \DIFadd{The use of symmetry adaption implies that the
present approach is valid only at large $R$ as in conventional SAPT,
\mbox{%DIFAUXCMD
\cite{doi:10.1063/1.463475} }\hspace{0pt}%DIFAUXCMD
but
...
\begin{figure}[hbtp]
  \DIFdelbeginFL %DIFDELCMD < \includegraphics{int_gold.eps}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \centering
  \includegraphics[width=0.5\linewidth]{int_gold.eps}
  \DIFaddendFL \caption{Diagrammatic representation of the A--B dispersion energy
    Eq. \eqref{eq:ecabfin}. \DIFdelbeginFL \DIFdelFL{Shaded }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Blue-shaded }\DIFaddendFL rings \DIFdelbeginFL \DIFdelFL{(blue) }\DIFdelendFL with upward--downward
    arrows denote particle--hole propagators of the monomers containing
    the full intra-monomer interaction, whereas horizontal \DIFdelbeginFL \DIFdelFL{wiggly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{wavy }\DIFaddendFL lines
    represent interactions between the monomers. Coupling strength and
    frequency integration \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are }\DIFaddendFL implied.}
  \label{fig:goldstone}
\end{figure}
...
\DIFaddbegin \subsubsection{\DIFadd{MP2C: Partial Resummation of MBPT}}

\DIFadd{He}{\DIFadd{\ss}}\DIFadd{elmann's MP2C method \mbox{%DIFAUXCMD
\cite{doi:10.1063/1.2905808,
  Pitonak10JChemTheoryComput6p168,doi:10.1063/1.4809981} }\hspace{0pt}%DIFAUXCMD
replaces
...
\DIFadd{in Eqs. }\eqref{eq:fdt}\DIFadd{-}\eqref{eq:alphac}\DIFadd{. Equivalently, MP2C may be
...
\boldsymbol{\kappa}^{\text{RPA}}(z)$ in Eq. }\eqref{eq:ecabrpa} \DIFadd{around
$\boldsymbol{\kappa}^{\text{RPA}}(z) = \mathbf{1}$, 
}\begin{equation}
  \DIFadd{\label{eq:ecabmp2c}
  E^{\text{C\, PT2C}}_{\text{int}}[\rho] = \frac{1}{4}
  \int_{-\infty}^{\infty} \frac{d\omega}{2\pi} \langle 
    \boldsymbol{\kappa}^{\text{RPA}}(z) - \mathbf{1} \rangle.
}\end{equation}
\DIFadd{In other words, MP2C includes intramonomer screening effects to infinite
...
in the L7 benchmark \mbox{%DIFAUXCMD
\cite{doi:10.1021/ct400036b}}\hspace{0pt}%DIFAUXCMD
.
...
little advantage compared to full RPA using Eq. }\eqref{eq:ecabrpa}\DIFadd{.
}

\DIFadd{Fig. \ref{fig:mp2c_rpa} summarizes the approximations to the AC-SAPT
dispersion energy discussed in this section in diagrammatic form.
}\begin{figure}[hbpt]
  \centering
  \includegraphics{int_gold_combine.eps}
  \caption{\DIFaddFL{Diagrammatic representations of the A--B dispersion energy
...
\DIFaddend \subsection{Numerical Validation}
...
$\boldsymbol{\Pi}_0$). Here we numerically evaluate \DIFaddbegin \DIFadd{the }\DIFaddend upper
bounds for 
$\alpha_c^{\text{PT2}}$ and investigate whether \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{these asymptotic bounds
}\DIFaddend can serve as meaningful convergence estimates \DIFaddbegin \DIFadd{for large but finite $R$}\DIFaddend . 
...
\begin{table}[hbpt]
  ...
  \begin{tabular}{lRR}
    \hline
    \text{Method} & \text{Slope (\DIFdelbeginFL \DIFdelFL{\%}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\%$}\DIFaddendFL /VE)} & y\text{-intercept (\%)} \\
    \hline
    \text{MP2} & 0.1219 & 7.30 \\
...
smallest and least polarizable systems. As a result, \DIFaddbegin \DIFadd{unscreened
}\DIFaddend perturbation theories 
produce exaggerated responses of the monomers to external perturbation \DIFdelbegin \DIFdel{,
producing 
}\DIFdelend \DIFaddbegin \DIFadd{and 
}\DIFaddend divergent estimates for NIs even in moderately large systems. The
conventional 
...
supported by the close agreement between the RPA and the benchmark results\DIFaddbegin \DIFadd{.
This agreement is remarkable given that RPA is parameter-free aside from
using a KS reference from a semilocal DFA}\DIFaddend . Moreover, the estimated
